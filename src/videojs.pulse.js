(function(vjs){
    ...
    var pulsePlugin = function(options, readyForPrerollCallback, adClickedCallback){
        ...
        var session = null;
        var readyCallback = readyForPrerollCallback;
        var vjsControls;
        var adPlayer;
        var adContainerDiv;
        ...
        if(!OO || !OO.Pulse){
            throw new Error("The Pulse SDK is not included in the page. Be sure to load it before the videojs plugin");
        }
        ...
        player.ready(function() {
            //Inform videojs that the plugin is ready
        ...
        player.pulse.initSession = function(sessionSettings){
            ...
            resetStates();
            //Register the relevant event listeners on videojs
        ...
        //Volume change listener for videojs
        function onVolumeChange(){
            var volume = player.muted() ? 0 : player.volume();
            adPlayer.setVolume(volume);
        }

        //fullscreen change listener for videojs
        ...
        //Get the HTML5 video element from videojs
        function getSharedElement(){
            //The videoJS video element has the id playerId_html5_api
        ...
        function readyForPreroll(){
            ...
            //If the integration has a custom callback for prerolls ready, call it, otherwise
            // play the session prerolls, if the session is ready
            if(readyCallback){
                readyCallback();
            } else {
                if(session){
        ...
        //Register the relevant event listeners for videohs
